...
// Функции распределений (с логгированием)
...
// Инициализация (с логгированием)
void InitializeParameters(DATA::Population& data, const Args& population_args) {
    data.mu = population_args.mutation_freq / population_args.locus_cnt;
    data.tint = round(static_cast<double>(population_args.full_time_in_epoch) / 10.0);
    data.fsample = 0.1;
}

void InitializeAncestryMatrix(DATA::Population& data, int N, int L) {
    data.A.assign(N, {});
    for (int i = 0; i < N; ++i) data.A[i].assign(L, i);
}

void InitializeDNA(DATA::Population& data, int N, int L, double f0) {
...
void ReserveMemoryForTemporalData(DATA::Population& data, int N, int L, int time_steps) {
...
void ReserveMemoryForStatistics(DATA::Population& data, int time_steps) {
...
void ReserveMemoryForNewGenerations(DATA::Population& data, int N, int L) {
    data.Knew.resize(N, vector<int>(L, 0));
    data.Anew.resize(N, vector<int>(L, 0));
    data.Pnew.resize(N, vector<int>(L, 0));
}

DATA::Population InitializePopulationData(const Args& population_args) {
    const double& s0 = population_args.adaptive_landscape;
    const int& L = population_args.locus_cnt;
    const int& N = population_args.population_cnt;
    const int& tf = population_args.full_time_in_epoch;
    const double& f0 = population_args.start_good_allele_freq;

    DATA::Population data;
    InitializeParameters(data, population_args);
    data.s.resize(L);
    DistributionSelection(population_args.dist_s, data.s, s0);
    InitializeAncestryMatrix(data, N, L);
    InitializeDNA(data, N, L, f0);

    int time_steps = tf;
    ReserveMemoryForTemporalData(data, N, L, time_steps);
    ReserveMemoryForStatistics(data, time_steps);
    ReserveMemoryForNewGenerations(data, N, L);

    Log("INIT", "InitializePopulationData", "completed");
    return data;
}
...
void PerformRecombination(vector<vector<int>>& K, vector<vector<int>>& A, vector<vector<int>>& P,
    int parent1, int parent2, const vector<bool>& use_parent1, int L) {
...
void CalculateAllStatistics(DATA::Population& data, const vector<double>& w, vector<vector<int>>& P, int t) {
...
// Основная функция эволюции (с логгированием)
DATA::Output RecombTrain(Args& population_args) {
    DATA::Population data = InitializePopulationData(population_args);
    DATA::Output output;

    double& s0 = population_args.adaptive_landscape;
    double& muL = population_args.mutation_freq;
    double& r = population_args.recomb_cnt;
    int& M = population_args.cross_cnt;
    int& N = population_args.population_cnt;
    int& L = population_args.locus_cnt;
    int& tf = population_args.full_time_in_epoch;
    double& f0 = population_args.start_good_allele_freq;
    ...
    for (int t = 0; t < tf; ++t) {
        if (t % data.tint == 0) {
            Log("PROGRESS", "RecombTrain", "generation " + to_string(t) + "/" + to_string(tf));
        }
        ...
        CalculateAllStatistics(data, w, P, t);
        ...
        if (t % data.tint == 0) {
            Log("OUTPUT", "RecombTrain", "computing fitness histogram");
...
int main() {
    ...
    monte_carlo_params.locus_cnt = 100;
    monte_carlo_params.population_cnt = 1000;
    monte_carlo_params.full_time_in_epoch = 300;
    monte_carlo_params.start_good_allele_freq = 0;
    monte_carlo_params.mutation_freq = 0.1;
    monte_carlo_params.exp_name = "experiment_1";

    Log("EXEC", "main", "starting RecombTrain");
    DATA::Output results = RecombTrain(monte_carlo_params);

    string base_name = "results_" + monte_carlo_params.exp_name;
    Log("SAVE", "main", "saving parameters");
    SAVE::Parameters(monte_carlo_params, results.theoretical_velocity, base_name + "_params.csv");

    Log("SAVE", "main", "saving statistics");
    SAVE::Statistics(results, monte_carlo_params, base_name + "_stats.csv");

    Log("SAVE", "main", "saving fitness histograms");
    SAVE::FitnessHistograms(results, base_name + "_histograms.csv");

    Log("SAVE", "main", "saving allele frequencies");
    SAVE::AlleleFrequencies(results, base_name + "_frequencies.csv");

    Log("EXEC", "main", "calling Python plotting script");
    string command = "python plot_results.py " + base_name;
    system(command.c_str());

    Log("MAIN", "main", "program completed");
    return 0;
}