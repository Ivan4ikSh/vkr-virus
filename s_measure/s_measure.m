...
function sdis = s_measure(genome1,genome2,genome3,order,C,appr,apprR,gen)
...
for param1= 1: length(xdots{1,times})
    ...
    if xdots{1,times}(param1)<=IallLen(times) && xdots{1,times}(param1)>=1 && imag(xdots{1,times}(param1))==0 && ydots{1,times}(param1)<=ybord{1,times} && ydots{1,times}(param1)>=ybord{2,times}
       ncount1 = ncount1 +1; 
...
hold on
plot(1:1:max(IallLen),zeros(1,max(IallLen)),'k--');
