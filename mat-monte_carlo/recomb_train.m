...
W=zeros(N,length(T));  
P1=zeros(N,length(T)); PL=P1; % Начальные метки родителей для 3 сайтов во времени, для филогении
...
%% Обновление популяции
   ...
   %% Рекомбинация случайно выбранных пар с заменой одного родителя
    npairs=round(r*N/2);
    ii=ceil(rand(npairs,2)*N); i1=ii(:,1); i2=ii(:,2);  % 2 столбца случайных индексов родителей
    for i=1:npairs
...
% Закрытие всех графиков (опционально)
% close all;
...
%% ФУНКЦИЯ ДЛЯ ВЫЧИСЛЕНИЯ ЧИСЛЕННОЙ СКОРОСТИ (V_num) - АЛЬТЕРНАТИВНАЯ ВЕРСИЯ
function V_num = compute_numerical_velocity(kav, T)
    % Вычисление численной скорости адаптации по данным kav(t)
    % kav - среднее число благоприятных аллелей во времени
    % T - временные точки
    
    % Используем линейную регрессию на последней трети данных
    t_start = round(2*length(T)/3);
    t_indices = t_start:length(T);
    
    if length(t_indices) > 1
        % Линейная регрессия kav(t) на интервале
        p = polyfit(T(t_indices), kav(t_indices), 1);
        V_num = p(1); % угловой коэффициент - скорость
    else
        % Если мало точек, используем простую разность
        V_num = (kav(end) - kav(1)) / (T(end) - T(1));
    end
